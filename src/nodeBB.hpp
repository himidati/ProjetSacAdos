...
public:
    ...
    void primalHeuristic(uint kpBound, uint nbItems, vector<uint> & weights,  vector<uint> & values);
    void solveUpperBound(uint kpBound, uint nbItems, vector<uint> & weights,  vector<uint> & values);
    void init(int size);
